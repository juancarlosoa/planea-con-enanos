using EscapeRoomPlanner.Application.Features.EscapeRooms.DTOs;
using EscapeRoomPlanner.Application.Features.EscapeRooms.Mappers;
using EscapeRoomPlanner.Application.Features.EscapeRooms.Queries;
using EscapeRoomPlanner.Domain.Interfaces;
using MediatR;

namespace EscapeRoomPlanner.Application.Features.EscapeRooms.Handlers;

public class GetAllEscapeRoomsHandler : IRequestHandler<GetAllEscapeRoomsQuery, IEnumerable<EscapeRoomDto>>
{
    private readonly IUnitOfWork _unitOfWork;

    public GetAllEscapeRoomsHandler(IUnitOfWork unitOfWork)
    {
        _unitOfWork = unitOfWork;
    }

    public async Task<IEnumerable<EscapeRoomDto>> Handle(GetAllEscapeRoomsQuery request, CancellationToken cancellationToken)
    {
        var escapeRooms = await _unitOfWork.EscapeRooms.GetActiveEscapeRoomsAsync(cancellationToken);
        return EscapeRoomMapper.ToDto(escapeRooms);
    }
}